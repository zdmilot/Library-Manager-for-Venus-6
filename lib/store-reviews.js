...
var _PROFANITY_LIST = [
    ...
    'dick','dumbass','fag','faggot','fuck','fucking','goddamn','hell',
    'jackass','motherfucker','nigger','nigga','piss','prick','pussy',
    'retard','retarded','shit','slut','twat','whore'
];
var _PROFANITY_RE = new RegExp('\\b(' + _PROFANITY_LIST.join('|') + ')\\b', 'i');

/**
 * Check text for profanity. Returns the matched word or null.
 */
function checkProfanity(text) {
    if (!text) return null;
    var m = _PROFANITY_RE.exec(text);
    return m ? m[1].toLowerCase() : null;
}

/**
 * Minimal spam filter. Returns a reason string if spam-like, or null if OK.
 * Checks both comment text and username for URLs / spam patterns.
 */
var _URL_TLDS = [
    'com','net','org','io','co','edu','gov','info','biz','xyz',
    'site','online','store','dev','app','me','us','uk','ca','de',
    'fr','ru','cn','ly','to','cc','tv','gg','link','click','shop',
    'pro','tech','top','zip'
];
// Standard domain: non-space immediately before dot, then TLD at word boundary
var _DOMAIN_RE = new RegExp('\\S\\.(' + _URL_TLDS.join('|') + ')\\b', 'i');
// Obfuscated domain: dot then optional spaces then spaced-out TLD letters (3+ char TLDs only)
var _SPACED_TLD_PARTS = _URL_TLDS.filter(function (t) { return t.length >= 3; }).map(function (tld) {
    return tld.split('').join('\\s+');
});
var _OBFUSCATED_RE = new RegExp('\\.\\s{0,5}(' + _SPACED_TLD_PARTS.join('|') + ')(?:\\s|$|[^a-zA-Z])', 'i');

function checkSpam(text, username) {
    if (!text && !username) return null;

    // Combine name + comment for URL scanning
    var combined = ((username || '') + ' ' + (text || '')).trim();

    // Block explicit URL protocols
    if (/https?:\/\//i.test(combined)) return 'URLs are not allowed in reviews';
    if (/\bwww\./i.test(combined)) return 'URLs are not allowed in reviews';

    // Block standard domain patterns (e.g. "google.com")
    if (_DOMAIN_RE.test(combined)) return 'URLs are not allowed in reviews';

    // Block obfuscated domains with spaced-out TLD (e.g. "g o o g l e . c o m")
    if (_OBFUSCATED_RE.test(combined)) return 'URLs are not allowed in reviews';

    // Only allow standard keyboard characters (printable ASCII + common punctuation)
    // This blocks emojis, unicode symbols, zero-width chars, etc.
    // Allowed: letters, digits, spaces, tabs, newlines, and standard keyboard symbols
    //   ! @ # $ % ^ & * ( ) - _ = + [ ] { } | \ ; : ' " , . < > / ? ~ `
    if (/[^\x20-\x7E\t\r\n]/.test(combined)) return 'Only standard keyboard characters are allowed';

    // Remaining spam checks apply only to comment text
    if (!text) return null;

    // Excessive caps (over 70% uppercase in text longer than 10 chars)
    if (text.length > 10) {
        var upper = text.replace(/[^A-Z]/g, '').length;
        if (upper / text.length > 0.7) return 'Excessive uppercase';
    }
    // Repeated same letter 4+ times in a row (e.g. "gooood", "niiiice")
    if (/([a-zA-Z])\1{3,}/i.test(text)) return 'Repeated letters';
    // Any single word longer than 30 characters (no spaces)
    if (/\S{31,}/.test(text)) return 'Word is too long';
    return null;
}
...
function submitReview(libraryName, username, rating, comment, systemInfo, cb) {
    ...
    if (!username || !username.trim()) return cb(new Error('Display name is required'));
    username = username.trim();
    if (username.length > 60) return cb(new Error('Display name is too long (max 60 characters)'));

    // Profanity check on name and comment
    var profaneName = checkProfanity(username);
    if (profaneName) return cb(new Error('Display name contains inappropriate language'));
    var profaneComment = checkProfanity(comment);
    if (profaneComment) return cb(new Error('Review contains inappropriate language'));

    // Spam check on comment
    var spamReason = checkSpam(comment, username);
    if (spamReason) return cb(new Error('Review flagged as spam: ' + spamReason));
