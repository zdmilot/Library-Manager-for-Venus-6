 ...
 *   - trust_status:   string   - 'trusted' | 'untrusted' | 'unknown' | 'none'
 *   - errors:         string[] - fatal verification failures
 *   - warnings:       string[] - non-fatal warnings
 *
 * @param {AdmZip}   zip
 * @param {Object}   [trustedCerts] - Map of fingerprint → cert for trust check
 * @returns {Object}
 */
function verifyPackageSignature(zip, trustedCerts) {
    var result = {
        valid: true, signed: false, code_signed: false,
        publisher_cert: null, trust_status: 'none',
        errors: [], warnings: []
    };
    ...
    if (sig.signature_format_version === '2.0' && sig.digital_signature && sig.publisher_certificate) {
        ...
        if (!certCheck.valid) {
            ...
            if (sigValid) {
                ...
                // Check trust status: auto-trust if publisher matches a known OEM name
                var pubIsOem = isRestrictedAuthor(cert.publisher) ||
                               isRestrictedAuthor(cert.organization);
                if (pubIsOem) {
                    result.trust_status = 'trusted';
                } else if (trustedCerts && typeof trustedCerts === 'object') {
                    if (trustedCerts[cert.fingerprint]) {
                        result.trust_status = 'trusted';
                    } else {
                        result.trust_status = 'untrusted';
                        result.warnings.push(
                            'Package is code-signed by "' + cert.publisher +
                            '" (key ' + cert.key_id + ') but this publisher is not a recognized OEM manufacturer.'
                        );
                    }
                } else {
...
/**
 * Load trusted publisher certificates from the publisher registry database.
 *
 * @param {string} registryPath - Path to publisher_registry.json
 * @returns {Object} Map of fingerprint → certificate object
 */
function loadTrustedCertificates(registryPath) {
    var certs = {};
    try {
        if (!fs.existsSync(registryPath)) return certs;
        var data = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
        var pubs = data.publishers || [];
        pubs.forEach(function(pub) {
            if (pub.certificates && Array.isArray(pub.certificates)) {
                pub.certificates.forEach(function(cert) {
                    if (cert.trusted && cert.fingerprint) {
...
/**
 * Add or update a trusted publisher certificate in the registry.
 *
 * @param {string}  registryPath  - Path to publisher_registry.json
 * @param {Object}  cert          - Publisher certificate object
 * @param {boolean} [trusted]     - Whether to mark as trusted (default: true)
 * @returns {boolean} true if saved successfully
 */
function saveTrustedCertificate(registryPath, cert, trusted) {
    if (typeof trusted === 'undefined') trusted = true;
    try {
        var data = { publishers: [], tags: [], maxPublisherSpaces: 0 };
        if (fs.existsSync(registryPath)) {
            data = JSON.parse(fs.readFileSync(registryPath, 'utf8'));
        }
        if (!data.publishers) data.publishers = [];

        // Find or create publisher entry
        var pubEntry = null;
        for (var i = 0; i < data.publishers.length; i++) {
            if (data.publishers[i].name === cert.publisher) {
                pubEntry = data.publishers[i];
                break;
            }
        }
        if (!pubEntry) {
            pubEntry = { name: cert.publisher, maxSpaces: 0 };
            data.publishers.push(pubEntry);
        }
        if (!pubEntry.certificates) pubEntry.certificates = [];

        // Update or add certificate
        var found = false;
        for (var j = 0; j < pubEntry.certificates.length; j++) {
            if (pubEntry.certificates[j].fingerprint === cert.fingerprint) {
                pubEntry.certificates[j] = Object.assign({}, cert, { trusted: trusted });
                found = true;
                break;
            }
        }
        if (!found) {
            pubEntry.certificates.push(Object.assign({}, cert, { trusted: trusted }));
        }
...
module.exports = {
    ...
    loadTrustedCertificates:      loadTrustedCertificates,
    saveTrustedCertificate:       saveTrustedCertificate,
