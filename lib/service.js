...
/**
 * Library Manager for Venus 6 - Service Layer  v1.6.5
 *
 ...
 *   - rest-api.js   (Swagger-documented REST API)
 *   - .NET COM      (VenusLibraryManager COM object shells out to REST/CLI)
 *
...
function getVENUSVersion() {
    ...
    try {
        ...
        for (var rp of regPaths) {
            try {
                var subkeysRaw = execSync('reg query "' + rp + '"', { encoding: 'utf8', timeout: 10000 });
                var subkeys = subkeysRaw.split('\n').map(function(l){return l.trim();}).filter(function(l){return l.length > 0;});
                for (var sk of subkeys) {
                    try {
                        var entryRaw = execSync('reg query "' + sk + '" /v DisplayName', { encoding: 'utf8', timeout: 5000 });
                        if (!/Hamilton\s+VENUS\s+\d/i.test(entryRaw)) continue;
                        var allVals = execSync('reg query "' + sk + '"', { encoding: 'utf8', timeout: 5000 });
 ...
 * @param {boolean} [opts.requireTrust=false]
...
function importLibrary(ctx, opts) {
    ...
    var trustedCerts = shared.loadTrustedCertificates(resolvePublisherRegistryPath());
    var sigResult = shared.verifyPackageSignature(zip, trustedCerts);
        ...
        if (sigResult.valid) {
            ...
            if (sigResult.code_signed && sigResult.publisher_cert) {
                if (sigResult.trust_status !== 'trusted') {
                    warnings.push('Package publisher is not in trusted registry');
                    if (!opts.force && opts.requireTrust) {
                        return { success: false, error: 'Package publisher is not trusted. Use force to import anyway.' };
                    }
                }
            }
        } else {
    ...
    var result = installPackage(manifest, zip, libDestDir, demoDestDir, path.basename(filePath), ctx.db, !!opts.noGroup);
...
function importArchive(ctx, opts) {
    ...
    pkgEntries.forEach(function(pkgEntry) {
        ...
        try {
            ...
            var libDestDir  = path.join(ctx.libBasePath, libName);
            var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', libName);
            var result = installPackage(manifest, innerZip, libDestDir, demoDestDir, label, ctx.db, !!opts.noGroup);
...
function exportLibrary(ctx, opts) {
    ...
    libraryFiles.concat(helpFiles).forEach(function(f) {
        ...
        if (fs.existsSync(fp2)) zip.addLocalFile(fp2, 'library');
    });
    demoFiles.forEach(function(f) {
        var fp3 = path.join(demoBasePath, f);
        if (fs.existsSync(fp3)) zip.addLocalFile(fp3, 'demo_methods');
    });
    ...
    else shared.signPackageZip(zip);
...
function createPackage(ctx, opts) {
    ...
    var libBasenames = resolvedLibFiles.map(function(f){return path.basename(f);});
    var helpBasenames = resolvedHelpFiles.map(function(f){return path.basename(f);});
    var manifestLibFiles = libBasenames.slice();
    helpBasenames.forEach(function(hf){ if (manifestLibFiles.indexOf(hf) === -1) manifestLibFiles.push(hf); });
    ...
    var manifest = {
        ...
        demo_method_files:    resolvedDemoFiles.map(function(f){return path.basename(f);}),
    ...
    var zip = new AdmZip();
    zip.addFile('manifest.json', Buffer.from(JSON.stringify(manifest, null, 2), 'utf8'));
    resolvedLibFiles.forEach(function(f){ zip.addLocalFile(f, 'library'); });
    resolvedHelpFiles.forEach(function(f){ zip.addLocalFile(f, 'library'); });
    resolvedDemoFiles.forEach(function(f){ zip.addLocalFile(f, 'demo_methods'); });
...
function rollbackLibrary(ctx, opts) {
    ...
    var result = installPackage(manifest, zip, libDestDir, demoDestDir, target.file, ctx.db, !!opts.noGroup);
    ...
    if (ext === '.hxlibarch') {
        ...
        pkgEntries.forEach(function(pkgEntry) {
            try {
                var innerZipBuf = shared.unpackContainer(pkgEntry.getData(), shared.CONTAINER_MAGIC_PKG);
                var innerZip = new AdmZip(innerZipBuf);
                var sigResult = shared.verifyPackageSignature(innerZip, trustedCerts);
                results.push({
                    ...
                    trust_status: sigResult.trust_status, errors: sigResult.errors, warnings: sigResult.warnings
                });
            } catch (e) {
        ...
        try {
            ...
            var sigResult2 = shared.verifyPackageSignature(zip, trustedCerts);
            results.push({
                ...
                trust_status: sigResult2.trust_status, errors: sigResult2.errors, warnings: sigResult2.warnings
            });
    ...
    Object.keys(baselineData.libraries).forEach(function(libName) {
        ...
        Object.keys(files).forEach(function(fname) {
            ...
            try {
                ...
                if (stored.checksum && footer.checksum !== stored.checksum) { results.tampered.push({ library: libName, file: fname, reason: 'Checksum changed' }); return; }
...
function generateKeypair(ctx, opts) {
    ...
    if (!opts.noTrust) {
        shared.saveTrustedCertificate(resolvePublisherRegistryPath(), cert, true);
    }
...
// 15. trustPublisher
// ---------------------------------------------------------------------------
function trustPublisher(ctx, opts) {
    opts = opts || {};
    if (!opts.certPath) return { success: false, error: 'certPath is required' };
    if (!fs.existsSync(opts.certPath)) return { success: false, error: 'Certificate file not found: ' + opts.certPath };

    var cert;
    try { cert = JSON.parse(fs.readFileSync(opts.certPath, 'utf8')); }
    catch(e) { return { success: false, error: 'Failed to parse certificate: ' + e.message }; }

    var certCheck = shared.validatePublisherCertificate(cert);
    if (!certCheck.valid) return { success: false, error: 'Invalid certificate: ' + certCheck.errors.join('; ') };

    var regPath = resolvePublisherRegistryPath();
    var trusted = !opts.revoke;
    var ok = shared.saveTrustedCertificate(regPath, cert, trusted);
    if (!ok) return { success: false, error: 'Failed to update publisher registry.' };

    return {
        success: true,
        data: { publisher: cert.publisher, organization: cert.organization || '', keyId: cert.key_id, fingerprint: cert.fingerprint, status: trusted ? 'trusted' : 'revoked' }
    };
}
