...
/**
 * Library Manager - Service Layer  v1.8.6
 *
 ...
 * @param {boolean} installToRoot - If true, files were installed to library root (no subdirectory).
 ...
 */
function installPackage(manifest, zip, libDestDir, demoDestDir, sourceName, db, skipGroup, sigResult, installToRoot) {
    var pathCheck = shared.validateManifestPaths(manifest);
    if (!pathCheck.valid) {
        throw new Error('Unsafe file paths in package manifest: ' + pathCheck.errors.join('; '));
    }
    ...
    // Auto-detect BMP library image from extracted files if the manifest has none
    var resolvedImageFilename = manifest.library_image        || null;
    var resolvedImageBase64   = manifest.library_image_base64 || null;
    var resolvedImageMime     = manifest.library_image_mime   || null;
    if (!resolvedImageBase64 && manifest.library_name) {
        var targetBmpName = (manifest.library_name || '').toLowerCase() + '.bmp';
        var bmpMatch = filteredLibFiles.find(function(f) {
            return path.basename(f).toLowerCase() === targetBmpName;
        });
        if (bmpMatch) {
            var bmpFullPath = path.join(libDestDir, bmpMatch);
            if (fs.existsSync(bmpFullPath)) {
                try {
                    resolvedImageBase64   = fs.readFileSync(bmpFullPath).toString('base64');
                    resolvedImageMime     = 'image/bmp';
                    resolvedImageFilename = path.basename(bmpMatch);
    ...
    var dbRecord = {
        ...
        library_image:       resolvedImageFilename,
        library_image_base64:resolvedImageBase64,
        library_image_mime:  resolvedImageMime,
        ...
        install_to_library_root: !!installToRoot
    };
    ...
    if (!saved || !saved._id) {
        throw new Error('Failed to save library record to database for "' + (manifest.library_name || 'Unknown') + '"');
    }
...
function importLibrary(ctx, opts) {
    ...
    if (importAuthor) {
        var svcAuthorCheck = shared.isValidAuthorName(importAuthor);
        if (!svcAuthorCheck.valid) return { success: false, error: svcAuthorCheck.reason };
    }
    {
        var svcOrgCheck = shared.isValidOrganizationName(importOrg);
        if (!svcOrgCheck.valid) return { success: false, error: svcOrgCheck.reason };
    }

    if ((shared.isRestrictedAuthor(importAuthor) || shared.isRestrictedAuthor(importOrg)) && !isSystemLibraryByName(libName)) {
        return { success: false, error: 'Restricted OEM author/organization. OEM packages can only be managed through the GUI with Developer Mode and OEM keywords enabled.' };
    }
    ...
    var svcInstallToRoot = !!opts.noSubdir || !!manifest.install_to_library_root;
    var libDestDir  = svcInstallToRoot ? ctx.libBasePath : path.join(ctx.libBasePath, libName);
    var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', libName);

    var result = installPackage(manifest, zip, libDestDir, demoDestDir, path.basename(filePath), ctx.db, !!opts.noGroup, sigResult, svcInstallToRoot);
...
function importArchive(ctx, opts) {
    ...
    pkgEntries.forEach(function(pkgEntry) {
        ...
        try {
            ...
            var importOrg    = (manifest.organization || '').trim();
            if (importAuthor) {
                var archAuthorCheck = shared.isValidAuthorName(importAuthor);
                if (!archAuthorCheck.valid) throw new Error(archAuthorCheck.reason);
            }
            {
                var archOrgCheck = shared.isValidOrganizationName(importOrg);
                if (!archOrgCheck.valid) throw new Error(archOrgCheck.reason);
            }
            if ((shared.isRestrictedAuthor(importAuthor) || shared.isRestrictedAuthor(importOrg)) && !isSystemLibraryByName(libName)) {
                throw new Error('Restricted OEM author/organization. OEM packages can only be managed through the GUI with Developer Mode and OEM keywords enabled.');
            }
            ...
            var archSvcInstallToRoot = !!opts.noSubdir || !!manifest.install_to_library_root;
            var libDestDir  = archSvcInstallToRoot ? ctx.libBasePath : path.join(ctx.libBasePath, libName);
            var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', libName);
            var result = installPackage(manifest, innerZip, libDestDir, demoDestDir, label, ctx.db, !!opts.noGroup, sigRes, archSvcInstallToRoot);
...
function createPackage(ctx, opts) {
    ...
    if (shared.isRestrictedAuthor(spec.author) || shared.isRestrictedAuthor(spec.organization)) {
        return { success: false, error: 'Restricted OEM author/organization. OEM packages can only be created through the GUI with Developer Mode and OEM keywords enabled.' };
    }
...
function rollbackLibrary(ctx, opts) {
    ...
    if ((shared.isRestrictedAuthor(rollbackAuthor) || shared.isRestrictedAuthor(rollbackOrg)) && !isSystemLibraryByName(rollbackLibName)) {
        return { success: false, error: 'Restricted OEM author/organization. OEM libraries can only be managed through the GUI with Developer Mode and OEM keywords enabled.' };
    }

    var rbSvcInstallToRoot = !!opts.noSubdir || !!manifest.install_to_library_root;
    var libDestDir  = rbSvcInstallToRoot ? ctx.libBasePath : path.join(ctx.libBasePath, opts.name);
    var demoDestDir = path.join(ctx.metBasePath, 'Library Demo Methods', opts.name);
    var result = installPackage(manifest, zip, libDestDir, demoDestDir, target.file, ctx.db, !!opts.noGroup, sigResult, rbSvcInstallToRoot);
    ...
    Object.keys(baselineData.libraries).forEach(function(libName) {
        ...
        Object.keys(files).forEach(function(fname) {
            ...
            try {
                ...
                // NOTE: Checksum comparison intentionally omitted — different VENUS versions
                // may rewrite footer checksums for valid files. Only the valid flag is checked.
...
function generateKeypair(ctx, opts) {
    ...
    if (shared.isRestrictedAuthor(opts.publisher) || shared.isRestrictedAuthor(opts.organization || '')) {
        return { success: false, error: 'Restricted OEM name. Key pairs for restricted OEM names can only be generated through the GUI with Developer Mode and OEM keywords enabled.' };
    }
